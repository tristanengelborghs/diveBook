...
class AddCardViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    var photoFront = Data()
    var photoFrontString: Data?
    let photoView2 = UIButton()
    var photoBack = Data()
    ...
    let diveSkilsSsi = ["Select dive skill","Scuba Diver", "Open Water Diver", "Enriched Air Nitrox","Night Diving", "Wreck Diving","Deep Divng", "Advanced Scuba Diver", "Rescue Scuba Diver", "Master Scuba Diver", "Dive Master", "Instructor"]
    ...
    func setupLayout() {
        ...
        photoView.layer.borderColor = UIColor.white.cgColor
        photoView.setTitle("photo of front", for: .normal)
        ...
        photoView2.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 30).isActive = true
        photoView2.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -30).isActive = true
        photoView2.titleLabel?.font = UIFont.init(name: "Avenir Next", size: 14)
        photoView2.layer.cornerRadius = 10
        photoView2.layer.borderColor = UIColor.white.cgColor
        photoView2.translatesAutoresizingMaskIntoConstraints = false
        photoView2.topAnchor.constraint(equalTo: photoView.bottomAnchor, constant: 20).isActive = true
        photoView2.heightAnchor.constraint(equalToConstant: 175).isActive = true
        photoView2.leadingAnchor.constraint(equalTo: self.scrollView.leadingAnchor, constant: 30).isActive = true
        photoView2.trailingAnchor.constraint(equalTo: self.scrollView.trailingAnchor, constant: -30).isActive = true
        photoView2.backgroundColor = .systemGray5
        photoView2.layer.borderWidth = 0
        photoView2.layer.borderColor = UIColor.white.cgColor
        photoView2.setTitle("photo of back", for: .normal)
        photoView2.addTarget(self, action: #selector(photoTapped), for: .touchUpInside)
        photoView2.contentMode = .scaleAspectFit
    ...
    @objc func saveCard(sender: UIButton!) {
        ...
        let PhotoFront = photoFrontString
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if (constant == 1) {
            ...
            photoFront = resize(image: finalImage!)!
            photoFrontString = photoFront
            ...
            photoView2.setImage(finalImage, for: .normal)
            photoView2.layer.borderWidth = 0
        }
    }
